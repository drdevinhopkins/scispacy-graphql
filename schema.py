...
class Token(ObjectType):
    ...
    shape = Int(
        description="Transform of token text, to show orthographic features")
    shape_ = String(
        description="Transform of token text, to show orthographic features")
...
class Span(ObjectType):
    ...
    start_char = Int(
        description="The character offset for the start of the span")
    ...
    negex = Boolean(
        description="Returns False if entity is negated based on NegEx rules")
...
class Doc(ObjectType):
    ...
    cats = List(
        Cat, description="The text classification categories, if available")
